...
**分数等级** (标准 — 资金流✅+LLM✅):
>=80 超强信号(strong_buy) | >=72 强信号(buy) | >=55 中等(watch) | <55 弱(skip)
...
熊市(bear): 量+价信号稀缺 → K线放宽阳线占比(1/4)保候选量，收紧波动/涨幅限制；S0板块扩展至top-10；L2量价与中性对齐
...
| 变量 | 说明 | 默认值 |
|------|------|--------|
| `L4_HIGH_THRESHOLD` | strong_buy 阈值 | `80.0` |
| `L4_BUY_THRESHOLD` | buy 阈值 | `72.0` |
| `L4_MEDIUM_THRESHOLD` | watch 下限 | `55.0` |
...
| 变量 | 说明 | 回测默认 | 实盘默认 |
|------|------|----------|----------|
| `BT_L3_MAX_VOLATILITY` | 波动率上限 (20日日线) | `0.60` | 0.50 |
...
| 推荐阈值 | strong_buy≥80/buy≥72/watch≥55 | strong_buy≥55/buy≥48/watch≥38 |
| 买入价 | 实盘最新价 | 当日收盘价 + 滑点 |
