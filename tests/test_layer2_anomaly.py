...
class TestLayer2Anomaly:
    def test_late_rally_passes(self):
        """尾盘拉升 > 2%, 放量 > 1.5x → 通过"""
        ctx = make_ctx(late_price_change=3.0, afternoon_volume_ratio=2.0)
    ...
    def test_breakout_passes(self):
        """突破日内高点 → 通过"""
        ctx = make_ctx(broke_high=True, afternoon_volume_ratio=2.0, late_price_change=0.5,
    ...
    def test_no_volume_fails(self):
        """无放量 → 不通过"""
        ctx = make_ctx(afternoon_volume_ratio=0.8, last_5min_volume_pct=3.0)
        result = screen_l2_anomaly([ctx])
        assert len(result) == 0
    ...
    def test_capital_check_optional(self):
        ...
        ctx = make_ctx(
            ...
            afternoon_volume_ratio=2.0,  # 但有放量拉升
